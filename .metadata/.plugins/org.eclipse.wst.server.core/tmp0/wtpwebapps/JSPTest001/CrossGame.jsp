...
var Yl = {
 ...
 ,init : function(){
  ...
  for(var i=0; i<this.nNum; i++){
   ...
   this.arrTopDiv[i].innerHTML = (i+1) + '<p><input type="button" value="시작" id="inp_top_'+i+'"class="btn btn-success" onclick="Yl.create('+i+');">'/*+'<br><input type="text" id="inp_top_'+i+'" value="" style="width:90%" tabindex="'+(i+1)+'" />'*/;
   if(i!=this.nNum-1){
	   this.arrBotDiv[i].innerHTML = '<input type="text" id="inp_bot_'+i+'" value="꽝" style="width:90%" tabindex="'+((i+1)+50)+'" />';
   }else{
	   this.arrBotDiv[i].innerHTML = '<input type="text" id="inp_bot_'+i+'" value="당첨" style="width:90%" tabindex="'+((i+1)+50)+'" />';
   }
 ...
 ,create : function(tn){ //사다리 생성
  ...
  //this.getEl("div_step2").innerHTML = "<center>" + '<a href="PointChangeSave.jsp?_point='+point+'&_type=game">포인트 집계 확인</a></center>';
   ...
   if( nEy<this.nHeight+100 ){
    ...
    for(var i=0; i<this.arrVerDiv.length; i++){
     if(nEx==parseInt(this.arrVerDiv[i].style.left)){
      ...
      this.arrGoDiv[pno].innerHTML = '<a href="PointChangeSave.jsp?_point='+point+'&_type=game">포인트 집계 확인</a></center>';
...
<input type="button" class="btn btn-success" value="만들기" onclick="Yl.init();">
